Hypothesize Core version *0.15.1* is now available from:

  <https://download.hypothesize.org/hypothesize-0.15.1/>
...
A number of changes to the way Hypothesize Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 hypo per kB (0.00001 IZE). This is to prepare for dropping the minimum transaction fee to 0.1 hypo per kB (0.0001 IZE) in the next release.
