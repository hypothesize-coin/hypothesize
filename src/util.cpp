...
const char * const BITCOIN_CONF_FILENAME = "hypothesize.conf";
const char * const BITCOIN_PID_FILENAME = "hypothesized.pid";
...
#else
    const char* pszModule = "hypothesize";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hypothesize";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hypothesize";
#else
    // Unix
    return pathRet / ".hypothesize";
#endif
#endif
}
